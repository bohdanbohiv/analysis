...
abbrev Nat.mul (n m : Nat) : Nat := Nat.recurse (λ _ prod ↦ prod + m) 0 n
...
theorem Nat.zero_mul (m : Nat) : 0 * m = 0 := recurse_zero (λ _ prod ↦ prod + m) _

/-- Definition 2.3.1 (Multiplication of natural numbers)
    Compare with Mathlib's {name}`Nat.succ_mul` -/
theorem Nat.succ_mul (n m : Nat) : (n++) * m = n * m + m := recurse_succ (λ _ prod ↦ prod + m) _ _
...
lemma Nat.mul_succ (n m : Nat) : n * m++ = n * m + n := by
  revert n
  apply induction
  · rw [add_zero, zero_mul, zero_mul]
  intro n h
  rw [succ_mul, succ_mul, add_assoc, add_comm m, succ_add, ←add_succ, ←add_assoc, h]
...
lemma Nat.pos_mul_pos {n m : Nat} (h₁ : n.IsPos) (h₂ : m.IsPos) : (n * m).IsPos := by
  obtain ⟨k, ⟨hn, _⟩⟩ := uniq_succ_eq n h₁
  obtain ⟨l, ⟨hm, _⟩⟩ := uniq_succ_eq m h₂
  rewrite [←hn, ←hm, succ_mul, mul_succ, hm]
  exact add_pos_right _ h₂
...
lemma Nat.mul_eq_zero (n m : Nat) : n * m = 0 ↔ n = 0 ∨ m = 0 := by
  constructor
  · contrapose!
    repeat rewrite [←isPos_iff]
    intro ⟨np, mp⟩
    exact pos_mul_pos np mp
  intro h
  obtain nz | mz := h
  · rewrite [nz]
    exact zero_mul _
...
theorem Nat.mul_lt_mul_of_pos_right {a b c : Nat} (h : a < b) (hc : c.IsPos) : a * c < b * c := by
  -- This proof is written to follow the structure of the original text.
  rewrite [lt_iff_add_pos] at h
  choose d hdpos hd using h
  replace hd := congr($hd * c)
  rewrite [add_mul] at hd
...
@[simp]
theorem Nat.pow_zero (m : Nat) : m ^ (0 : Nat) = 1 := recurse_zero (λ _ prod ↦ prod * m) _

/-- Definition 2.3.11 (Exponentiation for natural numbers) -/
@[simp]
theorem Nat.zero_pow_zero : (0 : Nat) ^ 0 = 1 := recurse_zero (λ _ prod ↦ prod * 0) _

/-- Definition 2.3.11 (Exponentiation for natural numbers)
    Compare with Mathlib's {name}`Nat.pow_succ` -/
theorem Nat.pow_succ (m n : Nat) : (m : Nat) ^ n++ = m ^ n * m :=
  recurse_succ (λ _ prod ↦ prod * m) _ _

/-- Compare with Mathlib's {name}`Nat.pow_one` -/
@[simp]
theorem Nat.pow_one (m : Nat) : m ^ (1 : Nat) = m := by
  rewrite [←zero_succ, pow_succ]; simp

/-- Exercise 2.3.4 -/
theorem Nat.sq_add_eq (a b : Nat) :
    (a + b) ^ (2 : Nat) = a ^ (2 : Nat) + 2 * a * b + b ^ (2 : Nat) := by
  rewrite [←one_succ, pow_succ, pow_one]
  ring_nf
  rfl
