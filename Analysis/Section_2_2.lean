...
abbrev Nat.add (n m : Nat) : Nat := Nat.recurse (λ _ sum ↦ sum++) m n
...
theorem Nat.add_left_cancel (a b c : Nat) (habc : a + b = a + c) : b = c := by
  ...
  intro a ih hbc
  rewrite [succ_add, succ_add] at hbc
...
theorem Nat.add_eq_zero (a b : Nat) (hab : a + b = 0) : a = 0 ∧ b = 0 := by
  ...
  obtain ha | hb := h
  · rewrite [←isPos_iff] at ha
    observe : (a + b).IsPos
    contradiction
  rewrite [←isPos_iff] at hb
  observe : (a + b).IsPos
  contradiction
...
example : (8 : Nat) > 5 := by
  rewrite [Nat.gt_iff_lt, Nat.lt_iff]
  constructor
...
theorem Nat.ge_antisymm {a b : Nat} (hab : a ≥ b) (hba : b ≥ a) : a = b := by
  rewrite [ge_iff_le, le_iff] at hab hba
  obtain ⟨n, hn⟩ := hab
  obtain ⟨m, hm⟩ := hba
  let h := hm
  rewrite [hn, add_assoc] at h
  nth_rw 1 [←add_zero b] at h
...
theorem Nat.add_ge_add_right (a b c : Nat) : a ≥ b ↔ a + c ≥ b + c := by
  ...
  · intro h
    ...
    obtain ⟨n, h⟩ := h
    use n
    rw [h, add_assoc, add_assoc, add_comm n c]
  intro h
  rewrite [ge_iff_le, le_iff] at *
  obtain ⟨n, h⟩ := h
  use n
  rewrite [add_assoc, add_comm c n, ←add_assoc, add_comm, add_comm _ c] at h
  apply add_left_cancel at h
  exact h
...
theorem Nat.lt_iff_succ_le (a b : Nat) : a < b ↔ a++ ≤ b := by
  ...
  · rewrite [le_iff, lt_iff]
    ...
    · unfold IsPos
      by_contra zero_n
      rewrite [zero_n, add_zero] at hn
      symm at hn
      contradiction
    let ⟨m, ⟨h, _⟩⟩ := uniq_succ_eq n n_pos
    simp only at h
    use m
    rw [hn, ←h, succ_add, add_succ]
  intro h
  rewrite [le_iff] at h
  obtain ⟨n, h⟩ := h
  rewrite [succ_add, ←add_succ] at h
  rewrite [lt_iff]
  constructor
  · use (n++)
  have n_succ_pos := succ_ne n
  by_contra b_eq_a
  rewrite [b_eq_a] at h
...
theorem Nat.lt_iff_add_pos (a b : Nat) : a < b ↔ ∃ d : Nat, d.IsPos ∧ b = a + d := by
  ...
  · intro h
    ...
    · unfold IsPos
      exact succ_ne n
    rw [h, succ_add, ←add_succ]
  intro h
  obtain ⟨n, ⟨n_pos, hn⟩⟩ := h
  let ⟨m, ⟨h, _⟩⟩ := uniq_succ_eq n n_pos
  simp only at h
  use m
  rw [succ_add, ←add_succ, h, hn]
...
theorem Nat.not_lt_of_gt (a b : Nat) : a < b ∧ a > b → False := by
  intro h
  have := ge_antisymm (le_of_lt h.2) (le_of_lt h.1)
...
theorem Nat.zero_le (a : Nat) : 0 ≤ a := by
  ...
  · exact le_refl 0
  intro a h
  have hs := succ_gt_self a
  rewrite [gt_iff_lt] at hs
...
theorem Nat.trichotomous (a b : Nat) : a < b ∨ a = b ∨ a > b := by
  ...
  revert a; apply induction
  · observe why : 0 ≤ b
    rewrite [le_iff_lt_or_eq] at why
    tauto
  intro a ih
  obtain case1 | case2 | case3 := ih
  · rewrite [lt_iff_succ_le] at case1
    rewrite [le_iff_lt_or_eq] at case1
    tauto
  · have why : a++ > b := by
      rewrite [case2]
    ...
    tauto
  have why : a++ > b := by
    rewrite [gt_iff_lt, lt_iff_add_pos] at *
    obtain ⟨d, ⟨d_pos, hd⟩⟩ := case3
    use (d++)
    constructor
    · unfold IsPos
      exact succ_ne d
...
/--
  ...
  followed by {syntax tactic}`exact Classical.decRel _`, but this would make this definition (as
  well as some instances below) noncomputable.
...
/-- An illustration of the {tactic}`calc` tactic with
    {kw (of := «term_≤_»)}`≤`/{kw (of := «term_<_»)}`<`. -/
...
lemma Nat.general_induction {n₀ : Nat} {P : Nat → Prop} (ih : ∀ n, n ≥ n₀ → P n → P (n++)) :
    P n₀ → ∀ n, n ≥ n₀ → P n := by
  let Q := λ n ↦ P (n + n₀)
  intro
  have h : ∀ n, Q n
  · apply induction
    ...
    rewrite [succ_add]
    apply ih
    · nth_rewrite 2 [←zero_add n₀]
      rewrite [←add_ge_add_right, ge_iff_le]
      exact zero_le _
    assumption
  intro n hn
  unfold Q at h
  rewrite [ge_iff_le, le_iff] at hn
  obtain ⟨a, ha⟩ := hn
  specialize h a
  rewrite [add_comm, ←ha] at h
  assumption

lemma Nat.lt_succ_iff_le {a b : Nat} : a < b++ ↔ a ≤ b := by
  rw [lt_iff_succ_le, ←one_add, ←one_add, ←add_le_add_left]
...
theorem Nat.strong_induction {m₀ : Nat} {P : Nat → Prop}
    ...
    ∀ m, m ≥ m₀ → P m := by
  have qn : ∀ n, ∀ m', m₀ ≤ m' ∧ m' < n + m₀ → P m'
  · apply induction
    · rewrite [zero_add]
      intro m hm
      have hm := not_lt_self (lt_of_le_of_lt hm.left hm.right)
      contradiction
    intro n hn m hm
    rewrite [succ_add, lt_succ_iff_le] at hm
    obtain hlt | heq := (le_iff_lt_or_eq _ _).mp hm.right
    · exact hn m ⟨hm.left, hlt⟩
    specialize hind (n + m₀)
    have h := zero_le n
    rewrite [←ge_iff_le, add_ge_add_right _ _ m₀, zero_add] at h
    replace hind := hind h
    replace h := hind hn
    rewrite [heq]
    exact h
  ...
  specialize qn (m++) m
  apply qn
  rewrite [←ge_iff_le, succ_add, lt_succ_iff_le]
  constructor
  · exact hm
  use m₀

lemma Nat.le_antisymm {a b : Nat} (hab : a ≤ b) (hba : b ≤ a) : a = b := by
  rewrite [←ge_iff_le] at hab hba
  exact ge_antisymm hba hab
...
theorem Nat.backwards_induction {n : Nat} {P : Nat → Prop}
  ...
  · intro p0 m hm
    convert p0
    exact le_antisymm hm (zero_le m)
  intro n ih psn m hm
  rewrite [le_iff_lt_or_eq] at hm
  obtain h | heq := hm
  · rewrite [lt_succ_iff_le] at h
    exact ih (hind n psn) m h
  rewrite [heq]
  exact psn
