...
example : (Real.pi + 1)^2 = Real.pi^2 + 2 * Real.pi + 1 := square_expand _
...
example (premise : ∀ m : Man, Mortal m) (Socrates : Man) : Mortal Socrates := by
  apply premise  -- `exact premise Socrates` would also work

end Remark_A_5_1

example : (∀ a : ℝ, ∀ b : ℝ, (a+b)^2 = a^2 + 2*a*b + b^2)
        = (∀ b : ℝ, ∀ a : ℝ, (a+b)^2 = a^2 + 2*a*b + b^2) := by
  rw [forall_comm]

example : (∃ a:ℝ, ∃ b:ℝ, a^2 + b^2 = 0)
        = (∃ b:ℝ, ∃ a:ℝ, a^2 + b^2 = 0) := by
  rw [exists_comm]
